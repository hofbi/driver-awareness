...
class RightTurn(BasicScenario):
    ...
    def _initialize_actors(self, config):
        ...
        second_vehicle = CarlaActorPool.request_new_actor(
            config.other_actors[1].model, config.other_actors[1].transform
        )
        self._other_actor_transform2 = config.other_actors[1].transform
        pedestrian = CarlaActorPool.request_new_actor(
            config.other_actors[2].model, config.other_actors[2].transform
        )
        self._pedestrian_transform = config.other_actors[2].transform
    ...
    def _create_behavior(self):
        ...
        move_pedestrian = KeepVelocity(self.other_actors[2], 1, distance=100)

        move_actor_parallel.add_child(move_pedestrian)
        # stop other actor
        ...
        sequence.add_child(
            ActorTransformSetter(self.other_actors[1], self._other_actor_transform2)
        )
        # sequence.add_child(
